...
const API_KEY= '59f1164e0f154cfaa41c5bc25704c3f7';
let savedArticles=[];
// news by source
intent('(Give|) (me|) (the|) (news|latest) from $(source* (.*))',(p)=>{
    ...
    api.request(NEWS_URL,(error,res,body)=>{
        ...
        p.play(`Here are the (recent|latest|current) news from ${p.source.value}`);
...
// News by Term
intent('what\'s up with $(term* (.*))',"news about $(term* (.*))", (p) => {
